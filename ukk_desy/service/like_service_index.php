...
if (mysqli_num_rows($checkLike) == 1) {
    while ($row = mysqli_fetch_array($checkLike)) {
        ...
        echo "
            <script>
                location.href='../admin/comment.php?picture={$_GET['pictureName']}&pictureTitle={$_GET['pictureTitle']}&pictureId={$_GET['pictureId']}&description={$_GET['description']}&uploadDate={$_GET['uploadDate']}';
            </script>
        ";
    }
} else {
